...
| Flutter baseline commit | `d366bca74f01619441f9a4cc3016efca183170e0` |
| Candidate state | Phase 0-7 source/static parity, Dart model/lifecycle hardening, LSP/network/security policy, package metadata hardening, P1 source-hardening, background Pigeon queues, native readiness cleanup, typed bridge-error mapping, typed Pigeon wire-envelope responses, native detach cleanup, clean consumer install/archive matrix, internal-beta supply-chain inventory/license/vulnerability/ABI gates, BIP340 official vector plus tamper/boundary coverage, secret-lifetime minimization, exact clean-candidate release-runner evidence, and local funded/unfunded iOS/Android runtime evidence were implemented against RN beta.26/core beta.6/RLN beta.3. Group 4 adds deterministic iOS artifact cache/offline acquisition, an honest iOS 18.5 minimum matching the pinned RLN object metadata, warning-free package dry-run evidence, podspec metadata validation policy, and expanded release/support docs. Group 5 adds strict analyzer settings for hand-written code, wallet contract type split, public collection defensive-copy boundaries, public API documentation and stale-language gates, and Dart coverage thresholds. Group 6 hardens the combined local release runner so platform runs include external-signer process-restart proofs and child reports compute release eligibility from pass plus clean-worktree state. Group 7 adds a machine-checked native artifact provenance manifest: Android's Maven AAR signature artifact is recorded as present but unverified, while the iOS GitHub release asset signature, per-artifact attestations, and reproducible-build evidence remain absent. Group 8 adds dirty-candidate lifecycle stress/fault tests and integration-smoke cleanup enforcement. The current dirty candidate also routes `UtexoWallet` node operations through `RLNBinding`, fails closed on old-artifact unfiltered transfer listing, documents/tests timestamp and amount units, hardens `reinit()` shutdown/create/unlock failure points, adds binding-level native-operation timeout quarantine semantics, fixes native store replacement state, proves native store cleanup on Android JVM plus iOS XCTest, removes the misleading Flutter platform-interface/method-channel shim, fails closed for standalone pure-Dart Schnorr signing unless explicitly experimental, makes wallet cleanup failures observable, removes message-substring native error classification, enforces zero analyzer diagnostics, and splits wallet/binding ownership into focused Dart parts. The source baseline, generated constants, docs, and parity validator now target RN beta.27/core beta.7 with live `origin/dev` drift detection. The current dirty candidate implements the beta.27/core beta.7 LSP/API/model deltas for `createLsp()`, `GET /get_info`, exact public network helpers, and BigInt unit helpers with targeted Dart vectors. Upstream production provenance/signatures/reproducibility and exact clean-candidate runtime proof remain pending |
...
| Flutter code currently targets | RN `1.0.0-beta.27`; core `1.0.0-beta.7`; RLN `0.10.0-beta.3`; current open rows are production supply chain and exact clean runtime evidence |
| Current RN native artifacts | RLN `0.10.0-beta.3` |
| Audit confidence | Static RN/source parity, artifact pinning, Dart LSP/network behavior, support-safe LSP diagnostics, package metadata checks, typed Pigeon wire-envelope transport, internal-beta supply-chain/license/vulnerability/ABI inventory, clean consumer install/archive builds, Android JVM bridge tests, iOS XCTest bridge tests, funded/unfunded platform smokes, external-signer process restart, Group 4 package/platform fixes, Group 5 codebase/docs/coverage hardening, Group 6 release-runner hardening, and Group 7 structured provenance gating were verified against beta.26/core beta.6/RLN 0.10.0-beta.3 on exact clean commit `bb1fe028c49ff307dcccfa68e4157623ccf2fce7`; Group 8 lifecycle/cleanup hardening plus current facade/lifecycle/unit/timeout/platform-abstraction/crypto-gating hardening are dirty-candidate evidence until committed and rerun. The current static RN/source validator now checks live RN `origin/dev` drift and passes against beta.27/core beta.7, but this is not runtime evidence and it does not close the beta.27 LSP/API/model rows. Release verdict remains NO-GO until all open tracker rows are verified or explicitly accepted |
...
production or mainnet funds. The source baseline is now refreshed to
beta.27/core beta.7/RLN 0.10.0-beta.3, the static RN parity validator passes
with live upstream-drift detection,
and the native artifact manifest verifies against the installed iOS files plus
downloaded iOS/Android release archives. This closes the stale-baseline and
missing-status/static-export failures, and phase 4-5 closes a meaningful slice
of Dart model/lifecycle defects, and phase 6-7 closes the Dart LSP/network plus
package metadata slice, and the P1 passes close repo-local toolchain pinning,
channel-domain mapping, integer fee-rate enforcement, Android artifact gating,
background Pigeon queues, native readiness cleanup, full on-chain receive
metadata, typed fee-rate response, typed optional carriers, bridge error
mapping, typed Pigeon wire-envelope responses, native detach cleanup, and
current funded/unfunded local platform smokes, clean consumer path/Git install
archives, internal-beta supply-chain/ABI inventory gates, BIP340 official
vectors, best-effort secret-lifetime minimization, release truth/divergence
policy, API/bridge snapshot gates, evidence cataloging, and sanitized unique
local report wrappers, Group 3 LSP/service negative coverage plus
reproducible license/vulnerability audit gates, Group 4 package/platform
minimum/support-matrix cleanup, and Group 5 codebase/docs/coverage hardening,
but there are still 2 open findings and no unresolved decisions.
Exact-current-candidate runtime evidence and upstream production provenance
remain release blockers. P0
completion removes
the known catastrophic
blockers; it does not convert the broader release verdict to GO.
...
not behavioral parity evidence, and the current platform smoke reports prove
only the covered funded/unfunded flows, not every API family or failure mode.
...
| `flutter test --coverage` | Pass, 145 tests; Dart package coverage 70.2% | Dart unit/contract tests pass and the Group 5 coverage floors pass; native RLN behavior is covered by the native/platform reports below |
| `flutter test` in `example/` | Pass, 1 test | Example widget smoke passes |
| `dart format --set-exit-if-changed lib test tool pigeons` | Pass, 44 files, 0 changed | Current source Dart files are formatter-canonical; generated release evidence under `build/` is analyzer/format excluded |
...
| `dart run tool/validate_test_matrix.dart` | Pass: 62 `RlnClient`, 107 `UtexoWallet`, 36 core exports | Validator proves local declaration coverage and implementation/evidence metadata, not full behavior |
...
| `dart run tool/validate_public_api_docs.dart` | Pass: 182 stable root symbols | Every root-exported stable symbol is listed in `doc/PUBLIC_API_REFERENCE.md`; the validator also forbids known advanced/RN/raw symbols from the stable root |
...
| `dart run tool/validate_coverage_policy.dart` | Pass | Dart coverage floors pass in the clean release run: package 70.2%, wallet 82.3%, LSP 67.7%, crypto 71.0%, models 74.8% |
| `dart run tool/validate_api_snapshot.dart` | Pass | Exported Dart API surface, Pigeon schema, generated bridge surface, and native bridge/store source markers match `tool/api_snapshot.json` |
| `RGB_SDK_RN_PATH=/Users/hardik/Projects/rgb-sdk-rn dart run tool/validate_rn_parity.dart` | Pass against live RN `origin/dev` `63cbf9a01030` | Static method names, low-level parameter/return categories, public wallet inventory, and runtime exports match the current beta.27 source. The validator now runs `git ls-remote origin refs/heads/dev`, checks fetched `origin/dev`, checks local `HEAD`, package/core versions, and RLN artifact pins before validating local source files |
...
| `dart pub publish --dry-run` | Root exits 65 with dirty-worktree warning; clean-consumer snapshot dry-run is warning-free | Package contents are intentional and no longer include vendored native binaries; commit before using root dry-run as final release evidence |
...
| `RUN_CONSUMER_ARCHIVES=1 tool/test_clean_consumer_matrix.sh` | Pass | Clean candidate snapshot passes tarball dry-run, path install, local Git install, Android release APK, iOS release `--no-codesign`, RLN artifact presence, and CocoaPods deployment/no-repo-update reinstall for both consumers; clean release-run report `build/test-reports/release/clean-consumer-matrix-20260805T161420Z-bb1fe02.json` |
...
| iOS XCTest bridge suite | Pass, 13 cases | Current source graph, exact storage policy, typed Pigeon wire envelope, detach cleanup, background Pigeon queue generation, and UUID destination path passed on simulator `8B3E2FF7-3A8C-4834-94E9-CC6AD707F2E5`; report `build/test-reports/release/native-ios-8B3E2FF7-3A8C-4834-94E9-CC6AD707F2E5-20260805T163000Z-bb1fe02.json` |
| Android JVM bridge suite | Pass, 17 cases | Current plugin/storage-policy, typed Pigeon wire envelope, detach cleanup, artifact verification, and request-boundary cases passed against RLN `0.10.0-beta.3`; report `build/test-reports/release/native-android-20260805T163000Z-bb1fe02.json` |
| iOS external-signer process restart | Pass | Clean run `20260805T163000Z`; one installed simulator binary crossed prepare/verify process boundary and passed trusted virtual BTC-channel restart proof; report `build/test-reports/release/external-signer-restart-8B3E2FF7-3A8C-4834-94E9-CC6AD707F2E5-20260805T163000Z-bb1fe028c49f.json` |
| Android external-signer process restart | Pass | Clean run `20260805T163000Z`; one installed APK crossed prepare/verify process boundary and passed trusted virtual BTC-channel restart proof on `emulator-5554`; report `build/test-reports/release/external-signer-restart-emulator-5554-20260805T163000Z-bb1fe028c49f.json` |
| Current general funded/unfunded platform smokes | Pass | iOS unfunded, iOS funded RGB+Lightning, Android unfunded, and Android funded RGB+Lightning smokes all passed from the exact clean candidate using the SDK regtest stack on RGB proxy port `3013`; reports are under `build/test-reports/release/` for run `20260805T163000Z` |
| Combined release-candidate script | Pass, release eligible on clean commit `bb1fe02` | Run `20260805T163000Z` passed package/static gates, clean consumer path/Git Android+iOS archives, native Android JVM, native iOS XCTest, iOS/Android unfunded smokes, iOS/Android funded RGB+Lightning smokes, and iOS/Android external-signer process restart; report `build/test-reports/release/release-candidate-20260805T163000Z-bb1fe02.json` has `releaseEligible: true` and `dirty: false` |
...
native storage behavior, the external-signer recovery invariant, and the
covered funded/unfunded platform smokes also have targeted iOS/Android
execution evidence. This does not prove complete native runtime correctness:
full bridge families, broader supported toolchain matrix, upstream production
provenance/signatures, and exact immutable release commit remain failed gates.
...
This section is the execution order for the 2 open findings. It groups only
...
The order is intentional: first harden quality and architecture seams that
could hide drift, then close supply-chain production mode, and finally rerun
the exact full native/platform release-candidate proof from a clean commit. Do not close a
group by saying a script passes unless the rows in that group have their own
...
| 2 | Exact current-candidate runtime evidence | TEST-028 | 1 | 1/0 | Runtime proof must be for the exact clean candidate after all code/docs changes, not an older commit. | Full local release-candidate runner passes from a clean commit with `releaseEligible: true`, `dirty: false`, and current iOS/Android native plus funded/unfunded reports attached. |

Coverage check: these groups cover 2 open rows total: 2 P1 and 0 P2. Any
...
| 2026-08-05 | Group 8 lifecycle stress and cleanup evidence | Partial verified | Added deterministic Dart lifecycle/fault tests for queued init/unlock/shutdown/destroy contention, failed-init temporary-node cleanup before retry, and destroy fault reporting after node cleanup. Hardened `example/integration_test/plugin_integration_test.dart` so low-level, facade, and funded regtest smokes always attempt node/wallet cleanup, delete their temporary storage directories, report cleanup failures with `RGB_SDK_FLUTTER_CLEANUP_ERROR`, and fail successful smokes if cleanup did not actually complete. Added the new lifecycle stress cases to the `unfunded_lifecycle` evidence bucket. Evidence for this dirty candidate: `flutter test test/utexo_wallet_test.dart`, full `flutter test` (148 tests), `dart run tool/validate_test_matrix.dart`, `dart run tool/validate_release_governance.dart`, `dart run tool/validate_release_package.dart`, `dart run tool/validate_release_language.dart`, `REPORT_DIR=build/test-reports/group-8 dart run tool/validate_supply_chain.dart`, `flutter analyze --no-fatal-warnings --no-fatal-infos`, and `git diff --check` pass. TEST-015 is verified. Native timeout/cancellation and native store cleanup completion are closed by the 2026-08-06 lifecycle slice. |
...
| G-09 Test evidence | Partial: exact clean runtime matrix exists for `bb1fe02`; Group 8 adds dirty-candidate Dart lifecycle stress and cleanup enforcement | Contract, native, restart, failure, funded/unfunded, and cross-platform tests pass on the exact release commit after the Group 7-8 changes are committed |
...
superseded by the ledger below. The remaining current gaps are:

| Area | Current status | Owning rows |
| --- | --- | --- |
| Release evidence | Clean release-candidate proof is for commit `bb1fe02`, before current dirty changes and before the current beta.27 LSP/API catch-up | TEST-028 |
...
| BASE-008 | P1 | Verified | Upstream RN `dev` is `63cbf9a01030a8a10eb1b04b2734cd8c5d23aec1`, package `1.0.0-beta.27`, core `1.0.0-beta.7`, and RLN `0.10.0-beta.3`; Flutter now targets the same source/artifact baseline and verifies generated constants, installed iOS files, iOS archive, Android AAR, and static RN parity. The last exact clean native/platform release evidence remains the older `bb1fe02` beta.26 candidate and is tracked under TEST-028. | Keep baseline pins and exact evidence refreshed whenever RN/core/RLN changes. |
...
| API-007 | P1 | Verified | Canonical Bitcoin/RGB wallet methods now return stable domain/core DTOs (`CoreBtcBalance`, `CoreUnspent`, `CoreListAssets`, `CoreAssetBalance`, `CoreInvoiceReceiveData`, `CoreInvoiceData`, `CoreTransaction`, `CoreTransfer`) while native `Rln*` DTOs are isolated behind explicit `Raw` methods. | Keep `*Core` aliases as compatibility names for the same stable domain result; do not add new canonical wallet methods that return `Rln*` DTOs. |
| API-008 | P1 | Verified | `onchainReceive` now preserves invoice, recipient ID, expiration timestamp, and batch transfer index through `OnchainReceiveResponse.fromRln`; the wallet contract test asserts every field. | Add native response-field assertions under TEST-005 before claiming full platform behavior coverage. |
| API-009 | P1 | Verified | Canonical `estimateFeeRate` now returns `RlnFeeRate`; the old scalar is isolated behind `estimateFeeRateValue`, and the wallet matrix plus unit test cover both. | Migrate callers to the typed response and deprecate the scalar alias under the public API policy. |
| API-010 | P1 | Verified | Node, network, peer, channel, Lightning payment, keysend, invoice-status, and decode wallet methods now map from native `Rln*` DTOs into stable wallet/domain DTOs (`WalletNodeInfo`, `WalletNetworkInfo`, `LightningPeer`, `LightningChannel`, `LightningPayment`, `LightningPaymentResult`, `LightningInvoiceStatus`, `DecodedLightningInvoice`). | Raw native shapes remain available through `nodeInfoRaw`, `networkInfoRaw`, `listPeersRaw`, `listChannelsRaw`, `listPaymentsRaw`, `getPaymentRaw`, `keysendRaw`, `invoiceStatusRaw`, and `decodeLnInvoiceRaw` for advanced parity/debugging only. |
...
| API-013 | P1 | Verified | `openChannel` now defaults `withAnchors` to true and the facade unit test asserts the native request field. | Add native platform request assertions under API-002/TEST-005. |
...
| API-032 | P1 | Verified | External-signer wallets expose false RGB UTXO/issuance capabilities, fail fast before native calls, and the approved trusted virtual BTC-channel external-signer regression now passes on the exact clean beta.26 candidate. | Exact clean run `20260805T163000Z` on commit `bb1fe028c49ff307dcccfa68e4157623ccf2fce7` produced release-eligible iOS and Android external-signer restart reports with `dirty: false`: `external-signer-restart-8B3E2FF7-3A8C-4834-94E9-CC6AD707F2E5-20260805T163000Z-bb1fe028c49f.json` and `external-signer-restart-emulator-5554-20260805T163000Z-bb1fe028c49f.json`. |
...
| MODEL-003 | P1 | Verified | Assignment kind, rotate/sign/verify/inflate response models, and `RlnUnspent.pendingBlinded` are represented and covered by facade/model tests. | Add platform fixture coverage under TEST-005/TEST-008 before claiming native behavior parity. |
...
| MODEL-006 | P1 | Verified | Current transaction cases `RGB_SEND`, `DRAIN`, `CREATE_UTXOS`, `SEND_BTC`, and `INCOMING` map to canonical core names, and unknown transaction types throw `NativeProtocolException` instead of collapsing to `User`. | Add native platform fixtures for every transaction case. |
| MODEL-007 | P1 | Verified | Transfer optionality is preserved for created/updated times and transport endpoints, and core mapping accepts `WaitingSafeHeight`, `Initiated`, and `Burn` while rejecting unknown status/kind values. | Add native platform fixtures for every transfer status/kind. |
...
| MODEL-009 | P1 | Verified | Lightning payment type/status mapping now returns canonical PascalCase values for payment APIs instead of uppercasing to wire-only vocabulary; invoice wire status stays uppercase until normalized by public helpers. | Add iOS/Android payment fixture vectors for every status/type. |
| MODEL-010 | P1 | Verified | `LightningChannel` now models the RN/core channel contract, `RlnChannel.toLightningChannel()` maps `public` to `isPublic`, normalizes channel status, preserves optional channel fields, and converts `localBalanceSat` to `localBalanceMsat`; `UtexoWallet.listLightningChannels()` and unit coverage exercise the mapper. | Add native platform channel fixtures under TEST-005/TEST-008 before claiming runtime behavior parity. |
...
| LIFE-005 | P1 | Verified | Wallet-facing chain, RGB, Lightning, channel, lookup, sync, and VSS calls now execute `_requireUnlockedNode()` before entering `RLNBinding`, while node-info remains initialized-only and disposed/shutdown states still fail before native calls. | Covered by `test/utexo_wallet_test.dart::requires unlock before chain, RGB, Lightning, and channel operations`; extend with any new wallet facade method. |
...
| LIFE-012 | P2 | Verified | Wallet public contract types/status helpers now live in `lib/src/wallet/utexo_wallet_types.dart`, `UtexoWallet` owns lifecycle/orchestration, native plugin files stay below the documented 1500-line guardrail, and `dart run tool/validate_codebase_hardening.dart` enforces the split/line-count boundary. | Deeper stable-facade contraction and raw-surface quarantine remain open under Group 1 rows; do not merge future DTOs back into `UtexoWallet`. |
...
| LSP-003 | P1 | Verified | `payAddress` now routes by address host: same-LSP addresses use the authenticated LSP client and do not fall back on auth/server errors; foreign addresses use LNURL discovery directly. | Add native/regtest LSP failure vectors before release evidence claims runtime parity. |
...
| LSP-005 | P1 | Verified | `claimPendingPayments` now reports missing preimages and skips the native `claimHodlInvoice` call instead of substituting an empty string; the unit test proves no native claim is recorded. | Add native fixture coverage for claimable payments with/without preimages. |
| LSP-006 | P1 | Verified | Settlement polling now calls `getLightningReceiveStatus` and normalizes Lightning invoice states instead of using RGB transfer-status compatibility helpers. | Remove stale status helpers under API-016. |
...
| PKG-007 | P1 | Verified | The podspec now bundles `ios/Resources/PrivacyInfo.xcprivacy`, and `validate_release_package.dart` gates the file plus resource bundle declaration. | Prove presence in a clean consumer app archive under PKG-016. |
...
| PKG-010 | P2 | Verified | `android/settings.gradle` was removed, leaving `android/settings.gradle.kts` as the single plugin settings file; package validation checks this invariant. | Commit deletion before relying on dry-run warning removal. |
...
| PKG-014 | P2 | Verified | Package contents are intentional and exclude vendored native artifacts. Root `dart pub publish --dry-run` reports only the expected dirty-worktree warning while this Group 4 patch is uncommitted; the clean-consumer matrix verifies a committed candidate snapshot so dry-run evidence is warning-free before release. | Keep dry-run warning-free before release and rerun it from the exact committed release candidate. |
...
| TEST-002 | P1 | Verified | The release candidate now has layered contract, native, and end-to-end runtime evidence on the exact clean commit: Dart/package tests, Android JVM bridge tests, iOS XCTest bridge tests, clean consumer path/Git archives, iOS/Android unfunded smokes, iOS/Android funded RGB+Lightning smokes, and iOS/Android external-signer restart proofs. | Exact clean run `20260805T163000Z` on commit `bb1fe028c49ff307dcccfa68e4157623ccf2fce7` produced `releaseEligible: true`, `dirty: false` in `build/test-reports/release/release-candidate-20260805T163000Z-bb1fe02.json`; rerun the all-up clean release script after committing the current dirty lifecycle/API slices. |
| TEST-003 | P2 | Verified | `doc/COVERAGE_POLICY.md` defines Dart-only coverage scope and generated-code exclusions; `tool/validate_coverage_policy.dart` gates package and domain thresholds. Exact clean run `20260805T163000Z` passes with 145 tests and coverage floors: package 70.2%, wallet 82.3%, LSP 67.7%, crypto 71.0%, models 74.8%. | Raise thresholds as behavior coverage improves; do not use Dart coverage as native/funds evidence. |
...
| TEST-013 | P1 | Verified | Android JVM bridge tests, iOS XCTest bridge tests, iOS unfunded, iOS funded, Android unfunded, Android funded, and the all-up `RUN_PLATFORM=1` local release-candidate script passed on dirty candidate `c131f50` and exact clean candidate `bb1fe02` against the current beta.26/RLN 0.10.0-beta.3 artifacts and regtest stack. | Keep rerunning this exact stack for every release-candidate commit; dirty reports remain non-release evidence. |
| TEST-014 | P2 | Verified | `doc/RELEASE_EVIDENCE_SCHEMA.md` defines evidence IDs, required fields, sanitization, and release eligibility. `tool/test_release_candidate.sh`, `tool/test_native_android.sh`, `tool/test_native_ios.sh`, `tool/test_platform_unfunded.sh`, `tool/test_platform_funded.sh`, and `tool/test_external_signer_restart.sh` now emit unique run-ID filenames with schema version, evidence ID, full commit, dirty state, sanitized repo-relative log path, and log SHA-256. Exact clean run `20260805T163000Z` produced release-eligible child reports with `dirty: false`. | Continue treating skipped gates and dirty-worktree runs as non-release-eligible. |
...
| TEST-016 | P2 | Verified | `tool/api_snapshot.json` snapshots the normalized exported Dart surface, Pigeon schema, generated Dart/Swift/Kotlin Pigeon surface, and native bridge/store source markers; `dart run tool/validate_api_snapshot.dart` gates drift. | Add model JSON/wire fixture snapshots when Group 2/3 finish domain-model cleanup and native behavior vectors. |
...
| CODE-002 | P2 | Verified | RN-shaped stable-root names are quarantined: `RgbSdkFlutter.rlnClient()`/`rlnManager()` were removed from the stable entrypoint, `RLNManager`/`RLNBinding`/`IRLN*` are advanced-only, and `decodeRGBInvoice`/`decodeRGBInvoiceCore` now exist only as an advanced compatibility extension while canonical stable wallet code uses `decodeRgbInvoice`. | Keep literal RN/native spellings out of the stable root unless a tracker row explicitly reclassifies them. |
